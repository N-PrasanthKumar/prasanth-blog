...
const Dashboard = () => {
 ...
 const handleDelete = async (postId) => {
  ...
  try {
    await postsAPI.deletePost(postId); // Call DeletePost API
    // Remove the deleted post from the dashboard state
    setDashboard(prev => ({
      ...prev,
      recent_posts: prev.recent_posts.filter(post => post.id !== postId),
      stats: {
        ...prev.stats,
        posts_count: prev.stats.posts_count - 1
      }
    }));
  } catch (error) {
  ...
  <div className="card text-center">
    <h3 className="stat-number">{dashboard?.stats?.posts_count || 0}</h3>
    <p className="stat-label">Posts Created</p>
  </div>
  <div className="card text-center">
    <h3 className="stat-number">{dashboard?.stats?.comments_made || 0}</h3>
    <p className="stat-label">Comments Made</p>
  </div>
  <div className="card text-center">
    <h3 className="stat-number">{dashboard?.stats?.comments_received || 0}</h3>
